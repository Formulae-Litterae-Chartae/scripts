...
def convert_docx_to_tei(input_docx_path:str):
    # https://teigarage.tei-c.org/ege-webservice/#/ege-webservice/doPost
    #/ege-webservice/Conversions/{input-document-type}/{output-document-type}
    #raise NotImplementedError
    import os
    import requests
    url = 'https://teigarage.tei-c.org/ege-webservice/Conversions/{input-document-type}/{output-document-type}'
    file_name = os.path.split(input_docx_path)[-1]
    file_content_type = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
    file_name = os.path.split(input_docx_path)[-1]
    
    files = {'fileToConvert': (file_name, open(input_docx_path, 'rb'), file_content_type)}
    
    # Documents:OpenOffice Text (.odt):odt,application/vnd.oasis.opendocument.text -> application:vnd.oasis.opendocument.text
    # Documents:Microsoft Word (.docx):docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document -> vnd.openxmlformats-officedocument.wordprocessingml.document
    # Documents:TEI P5 XML Document:TEI,text/xml -> TEI:text:xml
    #input_document_type="application:vnd.oasis.opendocument.text"
    input_document_type="TEI:text:xml"
    #output_document_type="TEI:text:xml"
    output_document_type="odt:application:vnd.oasis.opendocument.text"
    #https://teigarage.tei-c.org/ege-webservice/Conversions/odt%3Aapplication%3Avnd.oasis.opendocument.text
    r = requests.post(url="https://teigarage.tei-c.org/ege-webservice/Conversions/{}/{}".format(input_document_type, output_document_type), files=files, content_type="multipart/form-data")
    #r.raise_for_status()

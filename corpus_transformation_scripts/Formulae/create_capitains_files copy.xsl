...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:param name="mss-editions">
            ...
            <!-- <xsl:apply-templates select="$mssEditionFile/xml/formula[@n=concat($urn[1], '.', $urn[2], '.lat001')]"/> -->
            <!-- <xsl:value-of select="normalize-space($mssEditionFile/xml/formula[@n=concat($urn[1], '.', $urn[2], '.lat001')])"/> -->
            asdf
            <!-- <xsl:copy-of validation="lax" select="$mssEditionFile/xml/formula[@n=concat($urn[1], '.', $urn[2], '.lat001')]/node()"/> -->
            <xsl:copy-of select="$mssEditionFile/xml/formula[@n=concat($urn[1], '.', $urn[2], '.lat001')]/node()"/>
            <!-- <xsl:value-of select="unparsed-text(concat(replace($folderName, '/data/.*', ''), '/hss_editionen.xml'))"/> -->
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:call-template name="createCTS">
                                    ...
                                    <xsl:with-param name="mss-editions"><xsl:copy-of select="$mss-editions"/></xsl:with-param>
                                </xsl:call-template>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:if>
...
<!--                <xsl:if test="$isManuscript">
                ...
                <dct:references><xsl:copy-of select="$mss-editions"/></dct:references>
                <!-- <dct:references><xsl:value-of select="serialize($mss-editions, map{'method': 'xml', 'indent': 'no'})"/></dct:references> -->
            </structured-metadata>
        </xsl:param>

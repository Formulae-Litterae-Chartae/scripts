...
if not os.path.isdir(corpus_folder):
    raise SyntaxError("\n**The first argument must be a CapiTainS compatible corpus folder.{} is not a directory**\n".format(corpus_folder))
...
if os.path.isfile(inflected_to_lem_mapping):
    ...
    raise FileNotFoundError(inflected_to_lem_mapping+" not found")
logging.info(str(len(files))+' files found')

for f in sorted(files):
    logging.debug("Process {}".format(f))
...
if not os.path.exists(output_folder):
    ...
    logging.info('created: '+output_folder)

with open(output_file, mode="w+") as f:
    f.write('\n'.join(inflected_forms))
    logging.info("Success! All lemma where written to {}".format(output_file))
